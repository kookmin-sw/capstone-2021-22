...
class AutoRelater {
    ...
    addRelation(table, fkFieldName, spec, fkFields) {
        ...
        this.relations.push({
            ...
            childProp: isOne ? sequelize_1.Utils.singularize(childAlias) : sequelize_1.Utils.pluralize(childAlias),
        ...
        if (spec.isPrimaryKey) {
            ...
            const otherKey = lodash_1.default.find(fkFields, k => k.isForeignKey && k.isPrimaryKey && k.source_column !== fkFieldName);
            if (otherKey) {
                const otherModel = types_1.recase(this.caseModel, otherKey.foreignSources.target_table, this.singularize);
                const otherProp = this.getAlias(otherKey.source_column, otherKey.foreignSources.target_table, otherKey.foreignSources.source_table);
                const otherId = types_1.recase(this.caseProp, otherKey.source_column);
                this.relations.push({
                    parentId: sourceProp,
                    parentModel: targetModel,
                    parentProp: sequelize_1.Utils.pluralize(alias),
                    parentTable: types_1.qNameJoin(spec.foreignSources.target_schema || schema, spec.foreignSources.target_table),
                    childModel: otherModel,
                    childProp: sequelize_1.Utils.pluralize(otherProp),
    ...
    getAlias(fkFieldName, modelName, targetModel) {
